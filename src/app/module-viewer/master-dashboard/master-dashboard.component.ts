...
export class MasterDashboardComponent implements OnInit {
  ...
  ngOnInit() {
    this.organizations$ = combineLatest([
      this.moduleService.getOrganizations(true),
      this.sortOrder$,
      this.listSortOrder$,
      this.sortAsc$
    ]).pipe(
      map(([items, sortOrder, listSortOrder, sortAsc]) => {
        this.currentQuarter = items[0].current_quarter;
        const quarters = ['q1', 'q2', 'q3', 'q4'];
        this.quarters = quarters
          .concat(quarters)
          .slice(quarters.indexOf(this.currentQuarter) + 1, 8)
          .slice(0, 4);

        return items.sort((a, b) => {
          const direction = ('grid' === this.view
          ? sortAsc
          : listSortOrder.direction === 'asc')
            ? 1
            : -1;
          ...
          const field =
            'grid' === this.view
              ? sortOrder === 'Alphabetical'
                ? 'name'
                : 'progress'
              : listSortOrder.active;

          // keep rows with empty column values at the bottom of the table
          let defLowValue =
            (isNaN(parseInt(a[field], 10)) && isNaN(parseInt(b[field], 10))) ||
            'due_date' === field
              ? 'ZZZ'
              : 9999;
          ...
          return (aVal || defValue) < (bVal || defValue)
            ? -1 * direction
            : direction;
        });
      })
    ...
    this.gridHeight$ = combineLatest([
      ...
      map(([items, event]) => {
        ...
        const columns =
          100 /
          parseInt(
            window.getComputedStyle(el, null).getPropertyValue('max-width'),
            10
          );
        const containerHeight =
          (Math.ceil(items.length / columns) + 0.5) * el.clientHeight;
  ...
  viewAssessment(org: Organization, type: number) {
    this.router.navigate(['dashboard', org.id], {
      ...
      state: { section: 'assessments', type }
    });
  }
}
