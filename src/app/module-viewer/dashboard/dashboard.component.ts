...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  setOrganization(organization: Organization) {
    ...
    this.modulesRequest$ = this.moduleService
      .getCategories(this.organization.id)
      .pipe(shareReplay(1));

    this.assessmentScores$ = this.asmService
      .getModuleScores(organization.id)
      .pipe(shareReplay(1));

    this.modules$ = this.modulesRequest$.pipe(
      tap(
        response => (this.canActivate = this.canModifyPipe.transform(response))
      ),
    ...
    this.listModules$ = combineLatest([
      ...
      map(([response, listSortOrder, assessmentScores]) => {
        ...
        return items.sort((a, b) => {
          ...
          return (aValue || defValue) < (bValue || defValue)
            ? -1 * direction
            : direction;
        });
      })
