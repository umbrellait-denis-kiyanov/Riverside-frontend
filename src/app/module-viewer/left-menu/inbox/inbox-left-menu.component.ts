...
import {
  E3TableHeader,
  E3TableDataRow
} from 'src/app/common/components/e3-table';
...
export class InboxLeftMenuComponent implements OnInit {
  ...
  ngOnInit() {
    this.messages$ = this.inboxService
      .loadAll()
      .pipe(map(this.prepareData.bind(this)));
  }
  ...
  prepareData(messages: Message[]) {
    return messages.map(row => {
      ...
      message.onClick = () =>
        (message.className = message.className.split('pending').join(''));
