...
export class PreRequisiteModulesComponent  extends TemplateComponent {
  ...
  init() {
    this.contentData = this.data.data.template_params_json as PreRequisiteModuleTemplateData['template_params_json'];

    const moduleIDs = this.contentData.modules.map(module => Number(module.module));

    this.modules$ = this.navService.organization$.pipe(switchMap(orgId =>
      this.moduleService.getCategories(orgId).pipe(map(categories =>
        categories.body.reduce((res, category) =>
          res.concat(category.modules.filter(module => moduleIDs.includes(module.id))), [])
      ))
    ));
  }
  ...
  validate() {
    ...
    return this.modules$.pipe(
      map(modules => modules.every(module => module.status && module.status.progress === 100)),
