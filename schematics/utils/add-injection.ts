...
interface AddInjectionContext {
    ...
    relativeServiceFileName: string;    // e. g. ./core/side-riverside-template/side-riverside-template.service
...
function buildInjectionChanges(context: AddInjectionContext, host: Tree, options: ModuleOptions): Change[] {
    ...
    if (!ctorNode) {
        ...
        constructorChange = createConstructorForInjection(context, nodes);
    }
    else {
        constructorChange = addConstructorArgument(context, ctorNode);
    }
...
function addConstructorArgument(context: AddInjectionContext, ctorNode: ts.Node): Change {
...
function createConstructorForInjection(context: AddInjectionContext, nodes: ts.Node[]): Change {
